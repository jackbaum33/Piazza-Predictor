...
private:
  ...
  class PairComp {
    public:
    const bool operator() (const Pair_type& lhs, const Pair_type& rhs) const
    {
      return less(lhs.first,rhs.first);
    }
    Key_compare less;
  };
//students["jackbaum"]=67756475
public:
...
Map()
:values(BinarySearchTree<Pair_type,PairComp>()){}
  ...
  {
    return values.find({k,Value_type()});
  }
  ...
  Value_type& operator[](const Key_type& k)
  {
    ...
    Iterator valToFind=find(k);
    if(valToFind==values.end())
    {
      Iterator newVal=values.insert({k,Value_type()});
      return (*newVal).second; //find a way to return the second variable
    }
    else return (*valToFind).second;
  ...
  {
    Iterator firstFind=find(val.first);
    if(firstFind==values.end())
    {
      Iterator insert=values.insert(val);
      return {insert,true};
    }
    else return {firstFind,false};
  }
  ...
  Iterator end() const
  {
    Iterator i=values.end();
    return i;
  }

private:
  BinarySearchTree<Pair_type,PairComp> values;
};
