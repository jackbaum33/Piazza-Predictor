...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    if(empty_impl(node->left)) n->left=nullptr;
    else n->left=copy_nodes_impl(node->left);
    if(empty_impl(node->right)) n->right=nullptr;
    else n->right=copy_nodes_impl(node->right);
    return n;
  }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    if(!less(query,node->datum) && !less(node->datum,query)) return node;
    else
    {
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if(node->left)
    {
      if(less(node->left->datum,node->datum)) return true;
      else return false;
    }
    if(node->right)
    {
    if(less(node->datum,node->right->datum)) return true;
      else return false;
    }
  ...
  //       about where the element you're looking for could be.
    ...
    else if (less(node->datum, val))  
    {
      ...
      if ((!node->left)||less(node->left->datum, val)) return node;
      if((!less(node->left->datum, val) && !less(val, node->left->datum)))
      {
        return node;
      }
      else return min_greater_than_impl(node->left, val, less);
    }
    else {
      if (node->right) return min_greater_than_impl(node->right, val, less);
      else return nullptr;
    }
  }
